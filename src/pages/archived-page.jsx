...
export default function ArchivedPage() {
  ...
  const handleUnarchive = async (id) => {
    await updateTodo(id, { isArchived: false })
    fetchTodos()
  }
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{todos.length} archived todos</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-gray-500 mb-2">No archived todos</p>
            <button onClick={() => navigate('/')} className="text-xs text-violet-600 hover:underline">
              Back to todos
            </button>
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {todos.map(todo => (
              <div
                key={todo.id}
                className="bg-white border border-gray-200 rounded-xl px-4 py-3 flex items-center gap-3 group"
              >
                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => navigate(`/todo?id=${todo.id}`)}>
                  <p className="text-sm font-medium text-gray-600 line-through truncate">{todo.title}</p>
                  <p className="text-[11px] text-gray-400 mt-0.5">
                    {new Date(todo.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                  </p>
                </div>
