...
import { TODO_STATUS_STYLES,TODO_PRIORITY_COLORS } from "../types/todo-constant";
...
export default function TodoCard({ todo, onDelete }) {
  ...
  const isOverdue =
    ...
    todo.status !== "done";
  const updatedAt = new Date(todo.updatedAt).toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
    year: "numeric",
  });
  ...
  return (
    <div
      ...
      className={`bg-white border rounded-xl px-6 py-5 cursor-pointer hover:shadow-sm hover:border-gray-300 transition-all group
        ${
          todo.isPinned
            ? "border-l-[3px] border-l-violet-400 border-gray-200"
            : "border-gray-200"
        }`}
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <p
          className={`text-md font-medium leading-snug flex-1 min-w-0 truncate
          ${
            todo.status === "done"
      ...
      {/* <div className="flex items-center gap-1.5 flex-wrap mb-2">
        <span className={`text-[11px] px-2 py-1 rounded-full font-medium capitalize ${TODO_STATUSES[todo.status]}`}>
      ...
      <div className="flex items-center justify-between mb-3">
        <span
        className={`text-sm px-3 py-1 rounded-full font-medium ${TODO_STATUS_STYLES[todo.status]}`}
        >
          {todo.status?.replace("_", " ")}
        </span>
      ...
      {/* Priority, Checklist & Tag */}
      <div className="flex items-center gap-4 mb-3 text-sm text-gray-600">
        <div className="flex items-center gap-2">
          <span
            className={`w-3 h-3 rounded-full ${TODO_PRIORITY_COLORS[todo.priority]}`}
          ></span>
        ...
        {todo.tags?.slice(0, 1).map((tag) => (
          <span
            key={tag}
            className="px-2 py-1 rounded-full bg-violet-50 text-violet-600"
          >
            #{tag}
          </span>
        ))}
      ...
      <p className="text-[11px] text-gray-600">Updated {updatedAt}</p>
    </div>
  );
}
